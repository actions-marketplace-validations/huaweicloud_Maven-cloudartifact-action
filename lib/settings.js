...
function generateSettingXml(inputs) {
    const settingsXml = getTemplate('../templates', 'settings.xml');
    // 
...
function generateServersXml(settingsXml, servers) {
    ...
    JSON.parse(servers).forEach((server) => {
        const serverXml = getTemplate('../templates', 'servers.xml');
        serverXml.getElementsByTagName('id')[0].textContent = server.id;
        serverXml.getElementsByTagName('username')[0].textContent = server.username;
        serverXml.getElementsByTagName('password')[0].textContent = server.password;
